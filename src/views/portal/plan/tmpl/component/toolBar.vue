<template>
  <div class="main-style">
    <div class="tool-header" v-loading="loading">
      ...
      <span style="float:right;">
        <span v-for="(item, index) in toolsList" :key="index">
          <el-link v-if="item.disable" :underline="false" class="el-icon-circle-plus-outline icon-plus-style" disabled>
            <span>{{ item.name }}</span>
          </el-link>
          <el-link v-else :underline="false" class="el-icon-circle-plus-outline icon-plus-style" @click="goToAdd(item)">
            <span>{{ item.name }}</span>
          </el-link>
        </span>
      </span>
    ...
    <div class="content-style">
      <div v-for="(item, index) in contentsList" :key="index">
        <div
          v-if="item.type === '1'"
          class="item-main"
          style="line-height: 28px;padding-top: 16px;padding-bottom: 0px;"
        >
          <span
            style="width:calc(100%);display: inline-block;white-space:normal;word-wrap: break-word;word-break: break-all;"
          >
            <span class="span-title">{{ item.title }}:</span>
            <span class="span-content">{{ item.content }}</span>
          </span>
          <span style="display: inline-block; width: 100%; height: 40px;margin-top:-10px;">
            <i
              class="el-icon-delete"
              style="float:right;font-size: 20px;color:red;line-height: 40px;margin-left:12px; cursor:pointer;"
              @click="delOneSub(index)"
            ></i>
            <i
              class="el-icon-edit"
              style="float:right;font-size: 20px;line-height: 40px; cursor:pointer;"
        ...
        <div
          v-if="item.type === '2'"
          class="item-main"
          style="line-height: 28px;padding-bottom: 0px;padding-top: 16px;"
        >
          <el-row style="display:block;white-space:normal;word-wrap: break-word;word-break: break-all;">
          ...
          <el-row>
            ...
            <span class="span-title" style="margin-left: 32px;">责任人:</span>
          ...
          <el-row>
            <el-col :span="21">
              ...
              <span class="span-title" style="margin-left: 32px;">备勤人数:</span>
              <span class="span-content">{{ item.actorInNum }}人</span>

              <span class="span-title" style="margin-left: 32px;">到场时限:</span>
        ...
        <div v-if="item.type === '4'" class="item-main" style="padding-bottom: 0px;padding-top: 16px;">
          <span style="width:calc(100%);display: block;">
            <p style="display:block;white-space:normal;word-wrap: break-word;word-break: break-all;text-indent: 2rem;">
              {{ item.content }}
            </p>
          </span>
          <span style="display: block; width: 100%; height: 40px;margin-top: -10px;">
            <i
              class="el-icon-delete"
              style="float:right;font-size: 20px;color:red;line-height: 40px;margin-left:12px; cursor:pointer;"
              @click="delOneSub(index)"
    ...
    <businessDialog ref="dialogcontent" @refresh="refreshTable"></businessDialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    initforEdit(data) {
      ...
      if (this.toolBarData.stageType === 1) {
        this.toolsList = [
    ...
    refreshTable() {
      this.contentsList = [];
      this.loading = true;
      prePlanTmplApi.queryDatailorProcess({ planTmplItemId: this.toolBarData.stageId }).then(
        res => {
          this.loading = false;
    ...
    goToAdd(item) {
      ...
      this.dialogdata = {
        ...
        type: item.id, // 类型
        addstatus: true, //新建
        stageId: this.toolBarData.stageId // 详情id,
      };
    ...
    getDefaultDialogData(onedata, subid) {
      return {
        ...
        subid: subid,
        stageId: this.toolBarData.stageId
      };
    },
