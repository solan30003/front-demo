    ...
    >
      ...
      >
        <el-form-item v-if="dialogdata.type === '1'" label="名称:" prop="name">
          <el-input v-model="commandOrgan.name" style="width:300px;"></el-input>
        </el-form-item>
        <el-form-item v-if="dialogdata.type === '2' || dialogdata.type === '3'" label="指令内容:" prop="content">
          <el-input
            type="textarea"
            style="width:300px;"
            maxlength="200"
            :autosize="{ minRows: 5, maxRows: 30 }"
            v-model="commandOrgan.content"
            show-word-limit
          ></el-input>
        </el-form-item>
        <el-form-item v-show="dialogdata.type !== '4'" label="责任人:" prop="people">
          <el-cascader
            ...
            @change="organChanged"
          ></el-cascader>
          <div v-if="peopleOptions.length === 0 && selectedOrgan && !peopleNoSelect" style="color:red">
            该区域暂无人员！
          </div>
          <el-checkbox-group v-model="checkpeopleList" v-if="peopleOptions.length > 0 && selectedOrgan">
            <el-checkbox v-for="item in peopleOptions" :key="item.staffId" :label="item.staffId">{{
              item.name
            }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        ...
        <el-form-item v-if="dialogdata.type === '1'" label="备注:" prop="desc">
          <el-input
            ...
            maxlength="100"
            :autosize="{ minRows: 5, maxRows: 30 }"
            v-model="commandOrgan.desc"
            show-word-limit
          ></el-input>
        </el-form-item>

        <el-form-item v-if="dialogdata.type === '2' || dialogdata.type === '3'" label="到场限时:" prop="time">
          <el-col :span="6">
        ...
        <el-button style="margin-right:8px" type="primary" @click="submitcommandOrgan('commandOrgan')" size="small"
          >提交</el-button
        >
        <el-button style="margin-right:8px" size="small" @click="resetcommandOrgan('commandOrgan')">取消</el-button>
      </el-form>
      <!-- 文本 -->
      <el-form
        v-show="dialogdata.type === '4'"
      ...
      >
        ...
        <el-form-item label>
          <el-button style="margin-right:8px" type="primary" @click="submitText('addtext')" size="small"
            >提交</el-button
          >
...
export default {
  data() {
    ...
    var checkPeople = (rule, value, callback) => {
      if (!this.checkpeopleList.filter(res => res !== "undefined").length > 0) {
        this.peopleNoSelect = true;
        callback(new Error("请选择"));
      } else {
        this.commandOrgan.people = this.checkpeopleList.filter(res => res !== "undefined");
        callback();
      }
    };
    return {
      ...
      addtextRules: {
        text: [
          ...
          // { min: 0, max: 1024, message: "请输入长度在 0 到 1024 之间的字符", trigger: "blur" }
        ]
      },
      commandOrganRules: {
        name: [
          { required: true, message: "请输入名称", trigger: "blur" },
          { min: 1, max: 20, message: "请输入长度在 1 到 20 之间的字符", trigger: "blur" },
          { validator: checkName }
        ],
        content: [{ required: true, message: "请输入指令", trigger: "blur" }],
        // desc: [{ min: 0, max: 100, message: "请输入长度在 0 到 100 之间的字符", trigger: "blur" }],
        people: [{ required: true, message: "请选择", trigger: "blur" }, { validator: checkPeople }],
        arrivenums: [{ required: true, message: "请输入人数", trigger: "blur" }, { validator: checkNum }],
        backnums: [{ required: true, message: "请输入人数", trigger: "blur" }, { validator: checkNum }],
        time: [{ required: true, message: "请输入时间", trigger: "blur" }, { validator: checkNum }]
      },
      organsProps: {
        value: "id",
        label: "name",
  ...
  watch: {
    checkpeopleList(val) {
      this.commandOrgan.people = this.checkpeopleList.filter(res => res !== "undefined");
      let nameList = [];
      for (let one of this.allPeopleOptions) {
  ...
  methods: {
        ...
        commonApi.queryPeoplefromOrgan({ orgId: this.selectedOrgan }).then(
          res => {
            if (res.data.resultCode === 0) {
              ...
              this.allPeopleOptions.push(res.data.data.data);
    ...
    getPeopleNameById(id) {
      console.log("ssssssssssssssssssssssssssss", this.allPeopleOptions);
      for (let one of this.allPeopleOptions) {
        if (one.staffId === id) {
          return one.name;
        }
    ...
    addcommandOrgan() {
      ...
      let params = {
        ...
        // executorName: this.checkpeopleList
        //   .filter(res => res !== "undefined")
        //   .map(item => this.getPeopleNameById(item))
        //   .join(","),
